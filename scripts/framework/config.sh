...
{
    printf_step "/* Configuration Interface Header" \
                " * Auto-generated by config.sh for X3D Toggle v1.5" \
                " */"
    printf_br
    printf_step "#ifndef CONFIG_H" \
                "#define CONFIG_H"
    printf_br
    printf_step "#include \"xui.h\""
    printf_br
    printf_step "#define CONFIG_PATH      \"${_DAEMON_CONF}\"" \
                "#define GAMES_PATH       \"${_DAEMON_CONF}\"" \
                "#define DAEMON_CONF_PATH \"${_DAEMON_CONF}\""
    printf_br
    printf_step "#define CONFIG_POLLING_INTERVAL  ${POLLING_INTERVAL}" \
    ...
    printf_br
    printf_step "typedef struct {" \
                "2,int    polling_interval;" \
                "2,double refresh_interval;" \
                "2,int    dev_enable;" \
                "2,int    affinity_level;" \
                "2,char   affinity_mask[64];" \
                "2,double load_threshold;" \
                "2,int    detection_level;" \
                "2,int    ebpf_enable;" \
                "2,int    debug_enable;" \
                "2,char   daemon_state[32];" \
                "2,char   fallback_profile[64];" \
                "2,char   server_address[128];" \
                "} DaemonConfig;"
    printf_br
    printf_step "void config_load(DaemonConfig *cfg);" \
    ...
    printf_step "int cli_config_sync(int argc, char *argv[]);" \
